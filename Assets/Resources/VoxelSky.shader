// Unity built-in shader source. Copyright (c) 2016 Unity Technologies. MIT license (see VoxelSky.LICENSE.txt)

Shader "UnityCraft/Procedural Daylight Sky" {
// Based on Unity-Technologies/ShaderGraph-MasterStack-Samples/Assets/Shaders/Skybox-Procedural-Universal.shader
// Extension: a sun-directed horizon glow, independent of scattering wavelength tint.
Properties {
    _TwilightGlowColor ("Twilight Glow", Color) = (1, .34, .16, 1)
    _TwilightGlowStrength ("Twilight Strength", Range(0,1)) = 0
    [HideInInspector] _DaylightSunDirection ("Sun Direction", Vector) = (0,1,0,0)
...
SubShader {
    ...
    Pass {
        ...
        uniform half3 _TwilightGlowColor;
        uniform half _TwilightGlowStrength;
        uniform float4 _DaylightSunDirection;
        ...
        v2f vert (appdata_t v)
        {
            ...
            mainLight.direction = _DaylightSunDirection.xyz;
...
//              for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
        ...
        // if y < 0 [eyeRay.y > 0] - sky
            ...
            if(y < 0.0)
            {
                // Tint a broad band around the sun's horizon azimuth. Keep the built-in
                // scattering, sun disk, and stereo rendering, but do not retune wavelengths
                // to author a glow: that can turn the horizon green instead of orange.
                float3 viewDirection = -normalize(ray);
                float3 sunAzimuth = normalize(float3(mainLight.direction.x, .0001, mainLight.direction.z));
                half facing = pow(saturate(dot(viewDirection, sunAzimuth)), 4);
                half horizon = exp(-abs(viewDirection.y) * 9);
                half glow = _TwilightGlowStrength * facing * horizon;
                col = lerp(col, _TwilightGlowColor * .65h, glow * .85h);
